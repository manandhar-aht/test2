-- Simple schema for feature analysis data
-- This works with existing GeoServer layers

-- Create schema for nested feature data
CREATE TABLE IF NOT EXISTS feature_construction_details (
...
-- Create table for water quality reports
CREATE TABLE IF NOT EXISTS feature_water_quality_reports (
    id SERIAL PRIMARY KEY,
    feature_id VARCHAR(50) NOT NULL,
    feature_type VARCHAR(20) NOT NULL CHECK (feature_type IN ('well', 'pond', 'borehole')),
...
-- Create table for maintenance activities
CREATE TABLE IF NOT EXISTS feature_maintenance_activities (
    id SERIAL PRIMARY KEY,
    feature_id VARCHAR(50) NOT NULL,
    feature_type VARCHAR(20) NOT NULL CHECK (feature_type IN ('well', 'pond', 'borehole')),
...
CREATE INDEX IF NOT EXISTS idx_feature_construction_details_feature_id ON feature_construction_details(feature_id);
CREATE INDEX IF NOT EXISTS idx_feature_water_quality_reports_feature_id ON feature_water_quality_reports(feature_id);
CREATE INDEX IF NOT EXISTS idx_feature_water_quality_reports_test_date ON feature_water_quality_reports(test_date DESC);
CREATE INDEX IF NOT EXISTS idx_feature_maintenance_activities_feature_id ON feature_maintenance_activities(feature_id);
CREATE INDEX IF NOT EXISTS idx_feature_maintenance_activities_activity_date ON feature_maintenance_activities(activity_date DESC);
